...
- **`associatedReferences`** — литературный источник находки (библиографическая ссылка, глобальный уникальный идентификатор или URI).
...
Почти всегда в публикациях используются последовательности, опубликованные в GenBank — достаточно внести только их идентификаторы в формате `MT359934 | MT366828` (т.е. последовательности, полученные для разных генетических маркеров). В случаях, когда используются другие базы данных, это нужно отметить. Позднее, аудитор данных заменит идентификаторы на адреса вида `https://www.ncbi.nlm.nih.gov/nuccore/MT359934`.
